...
def test_plan_binds_every_assessed_byte_to_backup_or_quarantine(
    ...
    assert {entry.source_path for entry in plan.entries} == {
        ...
        *(stamp.path for stamp in assessment.evidence_files),
    }
    ...
    assert _entry(plan, "project.md").recovery_routes == ()
    assert _entry(plan, "state_current.md").destination_path == ("quarantine/state_current.md")
    assert _entry(plan, "state_current.md").recovery_routes == ()
    assert _entry(plan, "stack.md").recovery_routes == ()
    ...
    assert _entry(plan, "context/auth-cutover-2.md").recovery_routes == ()
...
def test_plan_preserves_ledger_and_unknown_bytes_without_resubmission(tmp_path: Path) -> None:
    binding = _binding(tmp_path)
    files = {
        ".decision-hashes.json": b'{"1":"old"}',
        ".sync-state.json": b"{}",
        "journal/log": b"evidence",
        "unknown.bin": b"\x00\xff",
    }
    for path, content in files.items():
        _write(binding.store_path, path, content)
    (binding.store_path / "empty-evidence").mkdir()
    assessment = assess_legacy_migration(_projection(binding, {}))
    plan = prepare_legacy_migration_plan(assessment)
    assert len(plan.entries) == len(files)
    for entry in plan.entries:
        assert entry.source_class == "evidence"
        assert entry.destination_path == f"legacy/{entry.source_path}"
        assert entry.sha256 == hashlib.sha256(files[entry.source_path]).hexdigest()
        assert entry.recovery_kind == "preserved_unsupported"
        assert entry.recovery_routes == ()
        assert entry.offer_export is True
    manifest = json.loads(plan.manifest_json)
    assert manifest["directory_paths"] == ["empty-evidence", "journal"]
    assert manifest["server_url"] == binding.server_url
    assert not plan.backup_root.exists()
