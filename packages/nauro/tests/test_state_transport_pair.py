...
def test_paired_state_transport(scenario):
    ...
    if not interpreter or not source:
        pytest.skip("Set NAURO_SERVER_PYTHON and NAURO_SERVER_ROOT for paired state transport.")
    root = Path(source).resolve()
    probe = Path(__file__).parent / "fixtures" / "state_transport_server.py"
    env = {
        **os.environ,
    ...
    result = subprocess.run(
        [
            ...
            "-p",
            "tests.conftest",
            f"{probe}::test_state_client_restart_over_tls[{scenario}]",
            "--tb=short",
        ],
        cwd=root,
        env=env,
