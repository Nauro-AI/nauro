...
def test_paired_share_transport(scenario):
    interpreter = os.environ.get("NAURO_SERVER_PYTHON")
    source = os.environ.get("NAURO_SERVER_ROOT")
    if not interpreter or not source:
        pytest.skip("Set NAURO_SERVER_PYTHON and NAURO_SERVER_ROOT for paired share transport.")
    root = Path(source).resolve()
    probe = Path(__file__).parent / "fixtures" / "share_transport_server.py"
    env = {
        **os.environ,
    ...
    result = subprocess.run(
        [
            ...
            "-p",
            "tests.conftest",
            f"{probe}::test_share_client_restart_over_tls[{scenario}]",
            "--tb=short",
        ],
        cwd=root,
        env=env,
