"""Truthful MCP share responses through the durable coordinator."""
...
from nauro.mcp import share_responses as responses
from nauro.store import share_records as records
from nauro.store.share_contract import ShareTransportError, verify_share_response
from nauro.store.submission_records import SubmissionRecordError
from nauro.sync import share_submission
from tests.test_share_submission import OTHER, _body, _prepared, _result, home
...
def test_noncommit_keeps_exact_evidence_and_truthful_text(home, status, text):
    ...
    action = responses.recover_share if status == "absent" else responses.submit_share
    ...
    assert records.read_share_submission(record.scope).result == result
...
def test_observation_then_commit_and_cached_receipt(home, status):
    ...
    assert response.content[0].text == (
        "Share operation committed. This receipt does not establish current local state."
    )
    assert responses.recover_share(record.scope, transport) == response
    assert responses.retry_share(record.scope, transport) == response
    assert responses.submit_share(record.scope, transport) == response
    assert transport.method_calls == [
        call.submit(
            records.ShareSubmission.model_validate({**record.model_dump(), "phase": "uncertain"})
        ),
        call.lookup(
            records.ShareSubmission.model_validate(
...
def test_recovery_never_submits_and_repeat_submit_does_not_resend(home):
    record = _prepared()
    transport = Mock(submit=Mock(return_value=_result(record, "slug_conflict_observed")))
    responses.submit_share(record.scope, transport)
    blocked = responses.submit_share(record.scope, transport)
    assert blocked.isError is True
    assert blocked.structuredContent is None
    transport.lookup.return_value = _result(record, "absent")
    assert responses.recover_share(record.scope, transport).structuredContent["unresolved"] is True
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ShareTransportError("secret response"),
...
def test_known_failure_does_not_disclose_or_claim_outcome(home, failure):
    ...
    response = responses.submit_share(record.scope, transport)
    assert response.isError is True
...
def test_failed_receipt_persistence_does_not_render_success(home, monkeypatch):
    ...
    monkeypatch.setattr(
        share_submission, "record_share_result", Mock(side_effect=OSError("final barrier"))
    )
    response = responses.submit_share(record.scope, transport)
    assert response.isError is True
    assert response.structuredContent is None
...
def test_invalid_receipt_is_not_rendered(home):
    ...
    response = responses.submit_share(record.scope, Mock(submit=Mock(return_value=invalid)))
    assert response.isError is True
    assert response.structuredContent is None
    assert records.read_share_submission(record.scope).phase == "uncertain"
