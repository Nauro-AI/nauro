"""Share identity durability and nonterminal observation recovery."""
...
from nauro.store.share_contract import ShareTransportError, share_payload, verify_share_response
...
def _prepared(slug="brief"):
    return records.prepare_share_submission(
        PROJECT, USER, share_payload(slug, "Frozen share", "brief", "Exact context")
    )
...
def _body(record, status="committed"):
    result = {
        ...
        "status": status,
        "unresolved": status in {"absent", "slug_conflict_observed"},
    }
    if status == "slug_conflict_observed":
        result.update(
            slug=json.loads(record.payload_json)["slug"],
            suggested_slug="different-slug",
        )
    if status == "committed":
        receipt = {
            "receipt_id": GENERATION,
            "operation_kind": "share_context",
            "operation_id": record.scope.operation_id,
            "result": "committed",
            "committed_at": "2026-09-06T00:00:00.000000Z",
            "artifact_digest": "a" * 64,
            "generation_id": GENERATION,
            "target_kind": "shared_brief",
            "target_id": json.loads(record.payload_json)["slug"],
            "details": {
                "base_generation_id": BASE,
                "base_manifest_digest": "a" * 64,
                "base_snapshot_digest": "a" * 64,
                "path": f"context/{json.loads(record.payload_json)['slug']}.md",
                "question_id": "Q1",
                "question_event_id": GENERATION,
                "question_created_at": "2026-09-06T00:00:00.000000Z",
...
def test_private_records_and_distinct_new_identities(home):
    ...
    changed = records.prepare_share_submission(
        PROJECT, USER, share_payload("brief", "Changed", "brief", "Exact context")
    )
    assert (
    ...
    assert path.is_relative_to(home / "submission-records" / "share")
    assert path.stat().st_mode & 0o777 == 0o600
    assert records.list_share_submissions(PROJECT, USER) == (first, second, changed)
    assert list_submissions(PROJECT, USER) == ()


@pytest.mark.parametrize("status", ["slug_conflict_observed"])
def test_observation_recovers_later_commit_without_resubmit(home, status):
    ...
    observed = submission.submit_share(record.scope, transport)
    assert observed.unresolved is True
    assert records.read_share_submission(record.scope).phase == "uncertain"
    with pytest.raises(submission.ShareRecoveryRequiredError):
        submission.submit_share(record.scope, transport)
    transport.lookup.return_value = _result(record)
    recovered = submission.recover_share(record.scope, transport)
    ...
    assert submission.recover_share(record.scope, transport) == recovered
...
def test_explicit_retry_looks_up_first_and_preserves_original_bytes(home):
    ...
    def send(saved):
        ...
        assert records.read_share_submission(record.scope) == saved
        assert saved.phase == "uncertain"
        return _result(record, "slug_conflict_observed")

    result = submission.retry_share(record.scope, Mock(lookup=lookup, submit=send))
    assert calls == ["lookup", "send"]
    assert result.unresolved is True


@pytest.mark.parametrize(
    "status", ["committed", "expired", "digest_conflict", "slug_conflict_observed"]
)
def test_retry_never_sends_for_non_absent_lookup(home, status):
    ...
    assert submission.retry_share(record.scope, transport) == _result(record, status)
...
def test_recovery_of_absence_never_sends(home):
    ...
    result = submission.recover_share(record.scope, transport)
    assert result.unresolved is True
...
@pytest.mark.parametrize("status", ["slug_conflict_observed"])
def test_lookup_slug_observation_prevents_resend(home, status):
    record = _prepared("a" * 64)
    transport = Mock(lookup=Mock(return_value=_result(record, status)))
    assert submission.retry_share(record.scope, transport).status == status
    transport.submit.assert_not_called()
...
def test_exact_resend_horizon(home, monkeypatch, elapsed, allowed):
    ...
    record = records.ShareSubmission.model_validate({**record.model_dump(), "created_at": created})
    ...
    if allowed:
        assert submission.retry_share(record.scope, transport).status == "committed"
    else:
        with pytest.raises(submission.ShareRetryExpiredError):
            submission.retry_share(record.scope, transport)
        transport.submit.assert_not_called()
    transport.lookup.assert_called_once()
...
def test_failed_namespace_barrier_is_repeated_before_network(home, monkeypatch):
    ...
    with pytest.raises(OSError):
        submission.submit_share(record.scope, transport)
    with pytest.raises(OSError):
        submission.recover_share(record.scope, transport)
    transport.submit.assert_not_called()
    ...
    assert submission.recover_share(record.scope, transport).status == "absent"
    transport.submit.assert_not_called()
...
def test_failed_receipt_persistence_leaves_recovery_available(home, monkeypatch):
    ...
    assert submission.recover_share(record.scope, transport).status == "committed"
    transport.submit.assert_called_once()


def test_native_lock_and_corruption_refuse_before_network(home, monkeypatch):
    record = _prepared()
    with records.share_submission_lock(record.scope):
    ...
    with pytest.raises(SubmissionRecordCorruptError):
        records.read_share_submission(record.scope)


def test_restart_recovers_observation_without_send(home):
    record = _prepared()
    submission.submit_share(
        record.scope, Mock(submit=Mock(return_value=_result(record, "slug_conflict_observed")))
    )
    script = """
import json,sys
from nauro.store.share_records import list_share_submissions
from nauro.store.share_contract import verify_share_response
from nauro.sync.share_submission import recover_share
record,=list_share_submissions(sys.argv[1],sys.argv[2])
class Transport:
...
result=recover_share(record.scope,Transport())
...
def test_response_binding_is_strict(home, field, value):
    ...
    with pytest.raises(ShareTransportError):
        verify_share_response(json.dumps(body).encode(), record.scope, record.payload_json)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("question_id", "bad"),
        ("question_event_id", "bad"),
        ("question_created_at", "2026-09-05T00:00:00.000000Z"),
        ("pointer_digest", "bad"),
...
def test_noncanonical_receipt_and_duplicate_response_keys_refuse(home):
    ...
    with pytest.raises(ShareTransportError):
        verify_share_response(duplicate.encode(), record.scope, record.payload_json)
...
def test_https_adapter_retains_identity_and_rechecks_account(home):
    ...
    with httpx.Client(transport=httpx.MockTransport(handler)) as client:
        ...
        assert str(calls[0].url) == "https://example.test/share/lookup"
...
def test_durable_send_and_receipt_barriers(home, monkeypatch):
    ...
    assert submission.submit_share(record.scope, Mock(submit=send)).status == "committed"
    assert events[-2:] == ["file", "directory"]
...
def test_invalid_payload_cannot_create_saved_identity(home):
    with pytest.raises(ValueError):
        records.prepare_share_submission(
            PROJECT, USER, b' {"delta":"x","expected_revision":null,"operation":"share_context"}'
        )
    assert records.list_share_submissions(PROJECT, USER) == ()


def test_resolved_record_cannot_be_reopened(home):
    record = _prepared()
    submission.submit_share(record.scope, Mock(submit=Mock(return_value=_result(record))))
    saved = records.read_share_submission(record.scope)
    with pytest.raises(SubmissionRecordError):
        records.mark_share_uncertain(saved)
    with pytest.raises(SubmissionRecordError):
...
def test_share_modules_have_no_production_consumers():
    ...
    modules = {
        "nauro.mcp.share_responses",
        "nauro.store.share_contract",
        "nauro.store.share_records",
        "nauro.sync.share_submission",
        "nauro.sync.share_transport",
    }
    found = []
...
def test_maximum_payload_round_trips_private_record(home, content):
    record = records.prepare_share_submission(
        PROJECT, USER, share_payload("a" * 120, content, "selection", "𐀀" * 300)
    )
    assert records.read_share_submission(record.scope) == record
...
def test_payload_change_requires_distinct_preparation(home, field, value):
    first = _prepared()
    payload = json.loads(first.payload_json)
    payload.pop("operation")
    payload[field] = value
    second = records.prepare_share_submission(PROJECT, USER, share_payload(**payload))
...
def test_discovery_is_separate_from_other_families(home):
    ...
    assert records.list_share_submissions(PROJECT, USER) == (record,)
...
def test_share_has_no_noop_observation(home):
    record = _prepared()
...
@pytest.mark.parametrize(
    "field,value",
    [("slug", "other"), ("suggested_slug", "brief"), ("suggested_slug", "../escape")],
)
def test_slug_observation_is_bound_and_advisory(home, field, value):
    record = _prepared()
    body = _body(record, "slug_conflict_observed")
    body[field] = value
    with pytest.raises(ShareTransportError):
        verify_share_response(
            json.dumps(body).encode(), record.scope, record.payload_json, lookup=True
        )


def test_verified_file_digests_are_not_reconstructed_from_pointer_text(home):
    record = _prepared()
    result = _result(record)
    details = json.loads(result.receipt_json)["details"]
    assert details["brief_digest"] == hashlib.sha256(b"Frozen share").hexdigest()
    assert details["pointer_digest"] == "d" * 64
    assert details["provenance_digest"] == "e" * 64


def test_slug_lookup_and_retry_preserve_original_identity(home):
    record = _prepared()
    transport = Mock(lookup=Mock(return_value=_result(record, "slug_conflict_observed")))
    observed = submission.recover_share(record.scope, transport)
    assert observed.unresolved is True
    assert submission.retry_share(record.scope, transport) == observed
    (saved,) = records.list_share_submissions(PROJECT, USER)
    assert saved.scope == record.scope
    assert saved.payload_json == record.payload_json
    assert saved.phase == "uncertain"
    transport.submit.assert_not_called()
...
@pytest.mark.parametrize(
    "key", ["question_id", "question_event_id", "pointer_digest", "provenance_digest"]
)
def test_missing_compound_receipt_fields_refuse(home, key):
    record = _prepared()
    body = _body(record)
    receipt = json.loads(body["receipt_json"])
    del receipt["details"][key]
    body["receipt_json"] = json.dumps(receipt, sort_keys=True, separators=(",", ":"))
    with pytest.raises(ShareTransportError):
        verify_share_response(json.dumps(body).encode(), record.scope, record.payload_json)
