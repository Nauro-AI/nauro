...
def generate_agents_md(
    ...
    *,
    ...
    ``manual_section`` and ``skills_section`` are emitted in ``section_order``, skills first.
    """
...
def _mask_header_timestamp(content: str) -> str:
    """Return ``text`` with the generated header timestamp line blanked, for content comparison.
    Valid only for files carrying ``HEADER_MARKER``: masking maps a marker line and a blank line
    to the same value.
    """
...
def remove_generated_agents_md(repo_path: Path) -> str | None:
    """Undo ``AGENTS.md`` generation in ``repo_path`` without destroying user content.
    A generated file is reduced to its non-empty ``# Manual`` section, else unlinked.
    Returns an indented status line, or ``None`` when nothing was Nauro-generated.
    """
