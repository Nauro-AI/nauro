...
class HooksMap(BaseModel):
    ...
    Only ``UserPromptSubmit`` is validated, as a JSON array of opaque entries bound to that
    exact alias; sibling events Nauro neither validates nor rewrites. Defaults rather than
    allowing ``None``, so an explicit JSON ``null`` raises into the EVENT_NOT_ARRAY skip.
    """
...
class ClaudeHookSettings(BaseModel):
    ...
    Only what Nauro touches is validated: ``hooks`` is a JSON object, and its
    ``UserPromptSubmit`` value is a JSON array when present. A non-object ``hooks``
    is routed to the HOOKS_NOT_OBJECT skip before this model runs.
    """
...
def _parse_hook_settings(raw: object) -> ClaudeHookSettings:
    """Validate ``raw`` into :class:`ClaudeHookSettings`, or raise a typed error.
    A non-object ``hooks`` is reported distinctly from an event value that is not an array.
    """
...
def _has_nauro_hook(settings: ClaudeHookSettings) -> bool:
    ...
    Non-dict matchers and non-nauro entries are skipped. The same :func:`_is_nauro_hook` predicate
    identifies the entry on the remove path, so add and remove agree.
    """
...
def _strip_hook_entries(raw: dict) -> bool:
    ...
    Preserves user hooks, matcher metadata and sibling events, dropping the installer-owned
    matcher shell only when it empties. Returns True when at least one entry was removed.
    """
    hooks = raw.get("hooks")
