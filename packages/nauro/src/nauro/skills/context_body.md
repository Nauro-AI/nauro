<!-- Source template. The dogfood files under .claude/, .cursor/, .agents/
     ...
     tools get_context, get_raw_file, and flag_question by name, alongside the
     agent's own filesystem write and the `nauro status` shell command, and
     makes no canonical protocol claims. -->
