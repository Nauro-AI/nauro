...
LegacySourceClass = Literal["identical", "divergent", "local_only", "snapshot", "evidence"]
...
LegacyRecoveryRoute = Literal["propose_decision"]
...
def _validate_assessment(assessment: LegacyMigrationAssessment) -> None:
    ...
    if not assessment.ready_for_backup:
        raise LegacyMigrationPlanError("Legacy migration planning requires settled pending paths.")
...
def _archive_entry(
    stamp: LegacyFileStamp, source_class: Literal["snapshot", "evidence"]
) -> LegacyMigrationPlanEntry:
    return LegacyMigrationPlanEntry(
        stamp.path,
        f"legacy/{stamp.path}",
        ...
        source_class,
        "legacy_backup",
        "archive_only" if source_class == "snapshot" else "preserved_unsupported",
        (),
        True,
    )
...
def prepare_legacy_migration_plan(
    ...
    project_id = assessment.projection.target.identity.project_id
    ...
    entries = tuple(
        sorted(
            ...
            + [_archive_entry(stamp, "snapshot") for stamp in assessment.snapshot_files]
            + [_archive_entry(stamp, "evidence") for stamp in assessment.evidence_files]
        )
    )
    payload = {
        ...
        "server_url": assessment.projection.target.binding.server_url,
        "directory_paths": assessment.directory_paths,
